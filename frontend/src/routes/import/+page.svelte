...
<script lang="ts">
	...
	// Импорт из GIFT — формат, которым уже пользуются курсы колледжа
	// (kst-test). Не всё конвертируется (вопросы на настоящее
	// соответствие ClassQuiz не поддерживает) — показываем отчёт вместо
	// молчаливой потери вопросов.
	let gift_file_input: File[] = $state();
	let gift_title = $state('');
	let gift_is_loading = $state(false);

	const gift_submit = async (e: Event) => {
		e.preventDefault();
		if (!gift_file_input || gift_file_input.length === 0) return;
		gift_is_loading = true;
		const formdata = new FormData();
		formdata.append('file', gift_file_input[0]);
		formdata.append('title', gift_title || gift_file_input[0].name.replace(/\.(gift|txt)$/i, ''));
		const res = await fetch('/api/v1/eximport/gift', {
			method: 'POST',
			body: formdata
		});
		gift_is_loading = false;
		if (res.status !== 200) {
			const body = await res.json().catch(() => ({}));
			alert(body.detail ?? 'Не удалось разобрать файл');
			return;
		}
		...
		if (data.skipped && data.skipped.length > 0) {
			const lines = data.skipped
				.slice(0, 15)
				.map((s: { name: string; reason: string }) => `• ${s.reason}`)
				.join('\n');
			alert(
				`Импортировано вопросов: ${data.imported_count}.\n` +
					`Пропущено: ${data.skipped.length} — добавьте их в редакторе вручную:\n\n${lines}`
			);
		}
		window.location.href = '/dashboard';
	};
...
<div class="flex items-center justify-center h-full px-4">
	<div>
		...
		<div
			class="lg:w-[80rem] lg:max-w-[80rem] w-screen max-w-screen mx-auto overflow-hidden bg-white rounded-lg shadow-md dark:bg-gray-800"
		>
			<div class="px-6 py-4">
				...
				<div class="grid grid-cols-1 md:grid-cols-3">
					...
					<form onsubmit={file_submit}>
						<div class="w-full mt-4 md:border-l-2 border-gray-600 h-full flex flex-col">
					...
					<form onsubmit={gift_submit}>
						<div class="w-full mt-4 md:border-l-2 border-gray-600 h-full flex flex-col">
							<h2 class="text-center text-2xl">Тест из GIFT</h2>
							<div class="dark:bg-gray-800 bg-white p-4 rounded-lg">
								<div class="relative bg-inherit w-full mb-3">
									<input
										id="gift-title"
										bind:value={gift_title}
										name="title"
										type="text"
										class="w-full peer bg-transparent h-10 rounded-lg text-gray-700 dark:text-white px-2 ring-2 ring-gray-500 focus:ring-sky-600 focus:outline-hidden"
										placeholder="Название теста"
									/>
								</div>
								<input
									id="gift-file"
									bind:files={gift_file_input}
									name="file"
									type="file"
									accept=".gift,.txt"
									class="w-full peer bg-transparent h-10 rounded-lg py-1.5 text-gray-700 dark:text-white ring-2 px-2 ring-gray-500 focus:ring-sky-600 focus:outline-hidden"
									class:ring-red-700={!gift_file_input}
									class:ring-green-600={gift_file_input}
								/>
								<p class="mt-2">
									Формат .gift, тот же, что уже используется в тестах курсов колледжа
									(kst-test). Вопросы на настоящее соответствие ClassQuiz не
									поддерживает — они попадут в отчёт после импорта, добавьте их
									вручную в редакторе.
								</p>
							</div>
							...
							<div class="flex items-center justify-center mt-auto">
								...
								<button
									class="px-4 py-2 leading-5 text-white transition-colors duration-200 transform bg-gray-700 rounded-sm hover:bg-gray-600 focus:outline-hidden disabled:cursor-not-allowed disabled:opacity-50"
									disabled={!gift_file_input || gift_is_loading}
									type="submit"
								>
									{#if gift_is_loading}
