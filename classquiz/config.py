...
class Settings(BaseSettings):
    ...
    mods: list[str] = []
    # Список username учёток с правами администратора платформы (управление
    # учётками, одобрение регистрации). НЕ путать с mods (модерация публичных
    # квизов) — это отдельная, более узкая роль из апстрима ClassQuiz.
    # Раньше "админом" считалась первая по дате создания строка в БД — это
    # было заменено на явный список именно для того, чтобы права нельзя было
    # получить случайно или порядком вставки строк.
    admins: list[str] = []
